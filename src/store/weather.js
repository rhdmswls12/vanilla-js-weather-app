...
export const searchWeather = async () => {
  ...
  const result = await fetch(
    `https://dapi.kakao.com/v2/local/search/address?query=${store.state.searchText}`,
    {
      method: "GET",
      headers: {
        Authorization: `KakaoAK ${process.env.PARCEL_LOC_KEY}`,
      },
    }
  );
  ...
  const resForAir = await fetch(
    `${urlForAir}serviceKey=${process.env.PARCEL_GEO_KEY}&returnType=json&sidoName=${sidoName}`
  );
  ...
  const res = await fetch(
    `${url}?serviceKey=${process.env.PARCEL_GEO_KEY}&dataType=json&numOfRows=10&pageNo=1&base_date=${dateFormat}&base_time=${baseTime}&nx=${nx}&ny=${ny}`
  );
...
function onGeoOk(position) {
  ...
  axios
    .get(
      ...
      { headers: { Authorization: `KakaoAK ${process.env.PARCEL_LOC_KEY}` } }
    )
    .then((res) => {
